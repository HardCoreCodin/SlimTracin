...
void drawCamera(Viewport *viewport, RGBA color, Camera *camera) {
    ...
    BoxCorners *corners = &viewport->default_box.vertices.corners;
    corners->back_bottom_left   = scaleVec3(corners->back_bottom_left,  0.5f);
    corners->back_bottom_right  = scaleVec3(corners->back_bottom_right, 0.5f);
    corners->back_top_left      = scaleVec3(corners->back_top_left,     0.5f);
    corners->back_top_right     = scaleVec3(corners->back_top_right,    0.5f);
    corners->front_bottom_left  = scaleVec3(corners->front_bottom_left,  2);
    corners->front_bottom_right = scaleVec3(corners->front_bottom_right, 2);
    corners->front_top_left     = scaleVec3(corners->front_top_left,     2);
    corners->front_top_right    = scaleVec3(corners->front_top_right,    2);
    for (u8 i = 0; i < BOX__VERTEX_COUNT; i++)
        viewport->default_box.vertices.buffer[i].z += 1.5f;
    drawBox(viewport, color, &viewport->default_box, &primitive, BOX__ALL_SIDES);
